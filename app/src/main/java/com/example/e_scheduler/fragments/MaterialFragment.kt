...
class MaterialFragment : Fragment(R.layout.fragment_material) {
    ...
    private fun getUpdatedList() {
        CoroutineScope(Dispatchers.Main).launch {
            val curUser = users.document(FirebaseAuth.getInstance().currentUser!!.uid).get().await()
                .toObject(User::class.java)!!
            val abc = materials.whereEqualTo("sem", curUser.semester)
                .whereEqualTo("branch", curUser.branch).get().await()
                .toObjects(Material::class.java) as ArrayList
            adapter = ResourceAdapter(requireContext(), abc)
            lv_resource.adapter = adapter
            adapter.setOnResourceClickListener {
